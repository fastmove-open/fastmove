...
/* ====================== Benchmarking ======================== */
static int nova_seq_bench_show(struct seq_file *seq, void *v)
{
	seq_printf(seq, "Echo use_dma(0/1):rw(0/1):page_type(0/1):size(KB, 1-2048):chan_node(0-(num_online_nodes-1)):chan_idx(0-7) to test PM copy bandwidth.\n"
			"    example: echo 0:1:0:64:1:3 > /proc/fs/NOVA/pmem0/bench\n");
	return 0;
}

static int nova_seq_bench_open(struct inode *inode, struct file *file)
{
	return single_open(file, nova_seq_bench_show, PDE_DATA(inode));
}

ssize_t nova_seq_bench(struct file *filp, const char __user *buf,
	size_t len, loff_t *ppos)
{
	struct address_space *mapping = filp->f_mapping;
	struct inode *inode = mapping->host;
	struct super_block *sb = PDE_DATA(inode);
	unsigned int use_dma, rw, page_type, size, chan_node, chan_idx;
	char * kbuf = memdup_user(buf, len);

	if (sscanf(kbuf, "%u:%u:%u:%u:%u:%u", &use_dma, &rw, &page_type, &size, &chan_node, &chan_idx) == 6)
		nova_bench(sb, use_dma, rw, page_type, size, chan_node, chan_idx);
	else
		nova_warn("Couldn't parse test_perf request: %s", kbuf);

	kfree(kbuf);
	return len;
}

static const struct proc_ops nova_seq_bench_fops = {
	.proc_open		= nova_seq_bench_open,
	.proc_read		= seq_read,
	.proc_write		= nova_seq_bench,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
void nova_sysfs_init(struct super_block *sb)
{
	...
	if (sbi->s_proc) {
		...
		proc_create_data("bench", 0444, sbi->s_proc,
				 &nova_seq_bench_fops, sb);
...
void nova_sysfs_exit(struct super_block *sb)
{
	...
	if (sbi->s_proc) {
		...
		remove_proc_entry("bench", sbi->s_proc);
