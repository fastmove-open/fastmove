...
static inline void *get_addr_from_offset(struct super_block *sb, u64 offset)
{
	struct nova_super_block *ps = nova_get_super(sb, offset);
	struct nova_sb_info *sbi = NOVA_SB(sb);

	int stripe_count = sbi->num_devices;
	u64 in_stripe_offset = offset % (PAGE_SIZE * sbi->stripe_size);
	int in_stripe_count = (offset - in_stripe_offset) / (PAGE_SIZE * sbi->stripe_size) / stripe_count;

	u64 translated_addr = in_stripe_count * (PAGE_SIZE * sbi->stripe_size) + in_stripe_offset;

	pr_warn_once("%s: This function is only used as a reference function, "
										"and for testing purposes only", __func__);

	if (!ps)
		return NULL;

	return ((void *)ps + translated_addr);
}
...
static inline void *nova_get_block(struct super_block *sb, u64 block)
{
	...
	pfn_t __pfn;
	struct nova_sb_info *sbi = NOVA_SB(sb);
	int in_page_offset = block % PAGE_SIZE;
	dax_direct_access(sbi->s_dax_dev, block / PAGE_SIZE,
					LONG_MAX / PAGE_SIZE, &virt_addr, &__pfn);
	virt_addr += in_page_offset;

#ifdef CONFIG_NOVA_DM_VERBOSE_CHECK
	do {
		void *dummy_virt_addr;
		dummy_virt_addr = get_addr_from_offset(sb, block);
		BUG_ON(dummy_virt_addr != virt_addr);
	} while (0);
#endif

	return block ? virt_addr : NULL;
}
...
{
	// FIXME: Expensive function invokation
	int i;
	int stripe_index = -1;
	int num_devices = sbi->num_devices;
	unsigned long size_each_stripe = sbi->single_device_size;
	void *curr_virt_addr;
	void **virt_addr_list = sbi->virt_addr_list;
	unsigned long offset_from_stripe;
	unsigned long offset_in_trunk;
	int trunk_num;

	for (i = 0; i < num_devices; i ++) {
		curr_virt_addr = virt_addr_list[i];
		if (addr >= curr_virt_addr && addr < curr_virt_addr + size_each_stripe) {
			stripe_index = i;
			break;
		}
	}
	...
	offset_from_stripe = addr - virt_addr_list[stripe_index];
	offset_in_trunk = offset_from_stripe % (sbi->stripe_size * PAGE_SIZE);
	trunk_num = (offset_from_stripe - offset_in_trunk) / (sbi->stripe_size * PAGE_SIZE);

	return (trunk_num * num_devices + stripe_index) * (sbi->stripe_size * PAGE_SIZE) + offset_in_trunk;
}
