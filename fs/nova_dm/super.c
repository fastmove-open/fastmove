...
{
	void **virt_addr_list = NULL;
	pfn_t *pfn_list = NULL;
	struct block_device **bdev_list = NULL;
	...
	size = dax_direct_access(sbi->s_dax_dev, 0, LONG_MAX/PAGE_SIZE,
				 &dummy_virt_addr, &__pfn_t) * PAGE_SIZE;

	size = (long) dax_get_size(sbi->s_dax_dev, &virt_addr_list, &pfn_list, &num_devices, &bdev_list)
						* PAGE_SIZE;

	sbi->num_devices = num_devices;
	sbi->virt_addr_list = virt_addr_list;
	sbi->bdev_list = bdev_list;

	// Init the stripe size. FIXME
	sbi->stripe_size = NOVA_STRIPE_PAGES;
	...
	// The dummy addr is to be set to the "PRIMARY" virt addr. In the future things like super
	// data will reside here
	sbi->virt_addr = dummy_virt_addr;

	sbi->phys_addr_list = kmalloc(sizeof(phys_addr_t) * num_devices, GFP_KERNEL);
	for (i = 0; i < num_devices; i ++)
		sbi->phys_addr_list[i] = pfn_t_to_pfn(pfn_list[i]) << PAGE_SHIFT;
	...
	sbi->initsize = size;
	sbi->single_device_size = size / num_devices;
	sbi->replica_reserved_inodes_addr = get_addr_from_offset(sb,
			size - (sbi->tail_reserved_blocks << PAGE_SHIFT));
	sbi->replica_sb_addr = get_addr_from_offset(sb, size - PAGE_SIZE);
	...
	printk("BBN: Info: \n");
	for (i = 0; i < num_devices; i ++) {
			printk("BBN: device %d: virt addr: %lx - %lx, phys_addr 0x%llx\n",
							i, (unsigned long)sbi->virt_addr_list[i],
							(unsigned long)sbi->virt_addr_list[i] + sbi->single_device_size,
							sbi->phys_addr_list[i]);
	}
...
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	struct nova_super_block *super = nova_get_super(sb, 0);
...
setup_sb:
	...
	node = dev_to_node(disk_to_dev(sb->s_bdev->bd_disk));
	nova_info("PMEM node: %d\n", node);
