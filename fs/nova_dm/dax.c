	...
	const char __user *buf,	size_t len, loff_t *ppos)
{
	...
	while (num_blocks > 0) {
		...
		printk("Write: %llx, len %lx\n", (unsigned long long)kmem, len);
#endif
		...
		case SYSCTL_FASTMOVE_DMA: {
			if (NOVA_SB(sb)->num_devices > 1) {
#ifdef CONFIG_NOVA_DM_VERBOSE_CHECK
				printk("A stripped target will require some translation here");
#endif
				// Do device translation here
				struct nova_sb_info *sbi = NOVA_SB(sb);
				stripe_index =
					get_index_from_offset(sb, blk_off);
				copied = bytes -
					 memcpy_to_pmem_nocache_fastmove(
						 sbi->bdev_list[stripe_index],
						 kmem + offset, buf, bytes);
			} else {
				copied = bytes -
					 memcpy_to_pmem_nocache_fastmove(
						 sb->s_bdev, kmem + offset, buf,
						 bytes);
			}
		} break;
	...
	unsigned int flags, struct iomap *iomap, bool taking_lock)
{
	...
	iomap->flags = 0;
	if (sbi->num_devices > 1) {
		stripe_index = get_index_from_offset(inode->i_sb, offset);
		iomap->bdev = sbi->bdev_list[stripe_index];
	} else {
