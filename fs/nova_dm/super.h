...
// FIXME: BBN: This is to be queriable in the future
#define NOVA_STRIPE_PAGES 512
...
struct nova_sb_info {
	...
	phys_addr_t	*phys_addr_list;
	void		*virt_addr;
	void		**virt_addr_list;
	void		*replica_reserved_inodes_addr;
	void		*replica_sb_addr;

	/* The number of devices in this fs */
	int num_devices;
	/* The size of a single device */
	unsigned long single_device_size;
	/* The size of stripe (if there is) */
	long stripe_size;
...
// FIXME: Dirty hack here, but could be useful
static inline int get_index_from_offset(struct super_block *sb, uint64_t offset) {
	struct nova_sb_info *sbi = NOVA_SB(sb);
	...
	stripe_index = (page_num / sbi->stripe_size) % sbi->num_devices;
 ...
 * Specify offset to get the block if it is a device mapper
 */
static inline struct nova_super_block *nova_get_super(struct super_block *sb, uint64_t offset)
{
	struct nova_sb_info *sbi = NOVA_SB(sb);

	int stripe_index;

	if (sbi->num_devices == 1 || offset == 0)
		return (struct nova_super_block *)sbi->virt_addr_list[0];

	stripe_index = get_index_from_offset(sb, offset);

	return (struct nova_super_block *)sbi->virt_addr_list[stripe_index];
}
