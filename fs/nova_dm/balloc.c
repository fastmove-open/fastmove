 ...
 *  The important flag marks whether the allocation is an
 *  important allocation. If the allocation is important, it
 *  will not fail and always allocates the same amount of blocks
 *  as reqested, even if it is a cross-PMEM allocation.
 *  So the important flag is to instruct the function to
 *  "allocate like it is a single-PMEM allocation"
 */
static long nova_alloc_blocks_in_free_list(struct super_block *sb,
	struct free_list *free_list, unsigned short btype,
	enum alloc_type atype, unsigned long num_blocks,
	unsigned long *new_blocknr, enum nova_alloc_direction from_tail, int important)
{
	struct rb_root *tree;
	...
	unsigned long step = 0;
	unsigned long max_stripe_cap = 0;
	unsigned long max_top = 0;
	unsigned long within_single_stripe = 0;
	unsigned long strip_page = NOVA_SB(sb)->stripe_size;

	bool single_device_alloc = important || NOVA_SB(sb)->num_devices == 1;
	...
	/* Note that if we are allocating for a device-mapper device, we are
	 * not going to allocation 2MB pages anyway */
	if (single_device_alloc &&
				IS_DATABLOCKS_2MB_ALIGNED(num_blocks, atype)) {
		found_hugeblock = nova_alloc_superpage(sb, free_list,
					num_blocks, new_blocknr, from_tail);
		if (found_hugeblock) {
#ifdef CONFIG_NOVA_DM_VERBOSE_CHECK
			pr_warn("Allocated 512 log blocks at %lx", *new_blocknr);
#endif
			goto success;
		}
	}

	// Make attempt to allocate important pages (which is always log pages)
	// in superpage
	if (important && atype == LOG && num_blocks ==  512) {
		pr_info_once("Allocating 512 log blocks, in super page");
		found_hugeblock = nova_alloc_superpage(sb, free_list, num_blocks,
	...
	while (temp) {
		...
		// If it is a stripe device allocation
		if (!single_device_alloc) {
			if (from_tail == ALLOC_FROM_HEAD) {
				max_top = (curr->range_low / strip_page + 1) * strip_page;
				max_stripe_cap = min(max_top - 1, curr->range_high);
				within_single_stripe = (curr->range_high <= max_stripe_cap);
			} else {
				max_top = (curr->range_high / strip_page) * strip_page;
				max_stripe_cap = max(max_top, curr->range_high);
				within_single_stripe = curr->range_high >= max_stripe_cap;
			}
		}

		// TODO: To prevent the bad side of single allocation,
		// we want to "overallocate"
		if (important && NOVA_SB(sb)->num_devices > 1) {}

		// For a striped device, we cannot allocate the whole block
		// if it is across the stripe boundary

		if (num_blocks >= curr_blocks
				&& (single_device_alloc || within_single_stripe)) {
		...
		// Note this might not allocate the number of blocks
		// equal to the requested
		if (!single_device_alloc) {
			if (from_tail == ALLOC_FROM_HEAD) {
				num_blocks = min(num_blocks, max_stripe_cap - curr->range_low + 1);
				*new_blocknr = curr->range_low;

				curr->range_low += num_blocks;
			} else {
				num_blocks = min(num_blocks, curr->range_high - max_stripe_cap);
...
alloc:
	ret_blocks = nova_alloc_blocks_in_free_list(sb, free_list, btype, atype,
					num_blocks, &new_blocknr, from_tail, important);
...
#ifdef CONFIG_NOVA_DM_VERBOSE_CHECK
	do {
		u64 start_offset = nova_get_block_off(sb, new_blocknr, btype);
		u64 end_offset = nova_get_block_off(sb, new_blocknr + ret_blocks - 1, btype);

		void *kmem_start = nova_get_block(sb, start_offset);
		void *kmem_end = nova_get_block(sb, end_offset);

		if (abs(kmem_end - kmem_start) != (ret_blocks - 1) * PAGE_SIZE) {
			pr_err("%s: kmem = %llx - %llx\n", __func__,
						(unsigned long long)kmem_start, (unsigned long long)kmem_end);
			pr_err("This allocation is cross-PMEM, you have been warned");
			if (!important) {
				BUG();
			}
		}
	...
	int important)
{
	...
	allocated = nova_new_blocks(sb, blocknr, num,
			    sih->i_blk_type, zero, DATA, cpu, from_tail, important);
			...
			int important)
{
	...
	allocated = nova_new_blocks(sb, blocknr, num,
			    sih->i_blk_type, zero, LOG, cpu, from_tail, important);
