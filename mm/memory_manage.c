...
static int migrate_to_node(struct list_head *page_list, int node,
		enum migrate_mode mode, int batch_size)
{
	...
	while (!list_empty(page_list)) {
		...
		if (migrate_concur)
			err = migrate_pages_concur(&batch_page_list, alloc_migration_target,
				NULL, (unsigned long)&mtc, mode, MR_SYSCALL);
		else
			err = migrate_pages(&batch_page_list, alloc_migration_target,
				NULL, (unsigned long)&mtc, mode, MR_SYSCALL);
		...
		struct list_head *page_list, unsigned long *nr_remaining_pages)
{
	...
	*nr_remaining_pages = 0;
	...
	if (max_nr_pages <= (2 * thp_nr_pages(page))) {
		max_nr_pages = 0;
	...
	struct list_head *exchange_list, unsigned long info_list_size)
{
	...
	while (!list_empty(from_pagelist) && !list_empty(to_pagelist)) {
		...
		if (thp_nr_pages(from_page) != thp_nr_pages(to_page)) {
			pr_info("from: %d, to: %d\n", thp_nr_pages(from_page), thp_nr_pages(to_page));
			if (!(thp_nr_pages(from_page) == 1 && thp_nr_pages(from_page) == HPAGE_PMD_NR)) {
				list_del(&from_page->lru);
				list_add(&from_page->lru, &odd_from_list);
			}
			if (!(thp_nr_pages(to_page) == 1 && thp_nr_pages(to_page) == HPAGE_PMD_NR)) {
		...
		unsigned long nr_pages, int flags)
{
	...
	bool migrate_fm = flags & MPOL_MF_MOVE_FM;
