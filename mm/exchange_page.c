...
int exchange_page_lists_mthread(struct page **to, struct page **from, int nr_pages) 
{
	...
	if (nr_pages < total_mt_num) {
		...
		if (residual_nr_pages) {
			for (i = 0; i < residual_nr_pages; ++i) {
				BUG_ON(thp_nr_pages(to[i]) != thp_nr_pages(from[i]));
				err = exchange_page_mthread(to[i], from[i], thp_nr_pages(to[i]));
				VM_BUG_ON(err);
			}
	...
	if (nr_pages < total_mt_num) {
		...
		for (item_idx = 0; item_idx < nr_pages; ++item_idx) {
			unsigned long chunk_size = nr_pages * PAGE_SIZE * thp_nr_pages(from[item_idx]) / total_mt_num;
			char *vfrom = kmap(from[item_idx]);
			char *vto = kmap(to[item_idx]);
			VM_BUG_ON(PAGE_SIZE * thp_nr_pages(from[item_idx]) % total_mt_num);
			VM_BUG_ON(total_mt_num % nr_pages);
			BUG_ON(thp_nr_pages(to[item_idx]) !=
				   thp_nr_pages(from[item_idx]));
		...
		for (i = 0; i < nr_pages; ++i) {
			...
			work_items[i].chunk_size = PAGE_SIZE * thp_nr_pages(from[i]);
